...
export const shell = {
     ...
     */
    run: (program: string, args?: string[], cancelId?: number, cwd?: string, rawArgs?: string): Promise<RunResult> => {
        const runId = _runSeq++;
        return new Promise<RunResult>((resolve) => {
            ...
            invoke('shell.run', { runId, program, args: args ?? [], cancelId: cancelId ?? -1, cwd: cwd ?? '', rawArgs: rawArgs ?? '' }).catch((e) => {
                off();
...
export const http = {
    ...
    /** Start a streaming (SSE) request. Listen via http.onStream for chunks tagged with the same id. */
    stream: (opts: {
        id: number;
        url: string;
        method?: string;
        headers?: Record<string, string>;
        body?: string;
    }) => invoke<{ ok: boolean }>('http.stream', opts),
    /** Cancel an in-flight streaming request by id. */
    streamCancel: (id: number) => invoke<{ ok: boolean }>('http.streamCancel', { id }),
    /** Subscribe to streaming events. Returns an unsubscribe function. */
    onStream: (handler: (e: StreamEvent) => void) => on<StreamEvent>('http.stream', handler),
};
