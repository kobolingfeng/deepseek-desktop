...
static void reg_http() {
    ...
    // Runs on a worker thread and pushes "http.stream" events to the frontend:
    //   { id, type:"open"|"chunk"|"done"|"error", data?, status?, error? }
    // Chunks are split on newline boundaries so each emitted "data" string is
    // always valid UTF-8 (a UTF-8 continuation byte is never 0x0A).
    ipc_on("http.stream", [](const json& a) -> json {
        int  id     = a.value("id", -1);
        auto url    = a.value("url", std::string{});
        auto method = a.value("method", std::string{"POST"});
        auto body   = a.value("body", std::string{});
        auto hdrs   = a.value("headers", json::object());
        if (id < 0)      throw std::runtime_error("id is required");
        if (url.empty()) throw std::runtime_error("url is required");
        if (!is_http_token(method)) throw std::runtime_error("Invalid HTTP method");

        // Validate + pre-format headers synchronously so bad input is rejected now.
        std::wstring allHeaders;
        for (auto& [k, v] : hdrs.items()) {
            if (!v.is_string() || !is_http_token(k) || has_header_injection_chars(v.get<std::string>()))
                throw std::runtime_error("Invalid HTTP header");
        ...
        std::thread([id, url, method, body, allHeaders, stream]() {
            ...
            [&]() {
                ...
                wchar_t host[256]{}, path[2048]{}, extra[2048]{};
                std::wstring objectPath;
                if (!crackHttpUrl(url, uc, host, path, extra, objectPath)) return fail("Invalid URL");
                if (uc.nScheme != INTERNET_SCHEME_HTTP && uc.nScheme != INTERNET_SCHEME_HTTPS)
                ...
                // Generous receive timeout: a reasoning model may pause before the first token.
                WinHttpSetTimeouts(hSession, 10000, 20000, 30000, 180000);
